...
public class ProductServiceImplement  implements ProductService{
    ...
    @Autowired
    public ProductServiceImplement(ProductRepository productRepository, @Lazy CategoryService categoryService,
	...
	public boolean createProduct(Product product) {
		try {
	       ...
	       // Log the new product entity
	        logger.info(String.format("Creating product: %s", newProductEntity.toString()));
	...
	public String deleteProduct(int productId) {
	    ...
	    if (removeProductEntity.isPresent()) {
	        ...
	        try {
	            ...
	            logger.info(String.format(">end>>>Deleted Product ID: %d", productId));
	            return String.format("The product with ID %d deleted successfully", productId);
	        } catch (DataIntegrityViolationException e) {
	            throw new ConflictException("Failed to delete product due to data integrity violation: " + e.getMessage());
	        } catch (Exception e) {
	            throw new RuntimeException("An unexpected error occurred while deleting the product: " + e.getMessage());
	        }
	    } else {
	        throw new NotFoundException(String.format("The product with ID %d not found.", productId));
	    }
	}
